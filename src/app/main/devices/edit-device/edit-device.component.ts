...
//   gateway: any[] = []; // Added to store the list of gateways
...
//     // Map initialization is now handled conditionally inside setActiveTab
//   }
...
//     forkJoin({
//       gateway: this.appService.getGateways(),
//       device: this.appService.getSingleDevice(devEUI)
//     }).pipe(take(1)).subscribe({
//       next: (results) => {
//         this.gateway = results.gateway.response;
//         const deviceData = results.device.response;
//         console.log('Fetched device data:', deviceData);
//         console.log('Fetched gateways:', this.gateway);
//         this.populateFormWithData(deviceData);
//       },
//       error: (error) => {
//         console.error('Error fetching data:', error);
//         alert('Failed to load device data or gateways. Check console for details.');
//         this.router.navigate(['/devices']);
//       },
//     });
//   }
...
//     // this.gateway = deviceData.gateway?.devEUI || null; // Pre-select the gateway
//     // this.selectedGatewayId = deviceData.gateway?._id?.$oid || null; 
//     this.gateway = deviceData.gateway?._id.$oid || null;
...
//     this.marker = L.marker([this.latitude, this.longitude]).addTo(this.map).bindPopup('Device Location').openPopup();
...
//         this.marker.bindPopup('Device Location').openPopup();
//       }
...
//   onLatLngChange(): void { this.updateMapMarker(); }

//   async searchLocation(): Promise<void> {
//     if (!this.searchAddress) { console.warn('Please enter an address to search.'); return; }
...
//       } else { console.warn('Location not found for the given address.'); }
//     } catch (error) { console.error('Error searching location:', error); }
//   }

//   generateDevEUI(): void { this.devEUI = Array.from({ length: 16 }, () => Math.floor(Math.random() * 16).toString(16)).join('').toUpperCase(); }
//   generateAppKey(): void { this.appKey = this.generateRandomHexString(32); }
//   generateDevAddr(): void { this.devAddr = this.generateRandomHexString(8); }
//   generateNwkSKey(): void { this.nwkSKey = this.generateRandomHexString(32); }
//   generateAppSKey(): void { this.appSKey = this.generateRandomHexString(32); }
...
//     return result;
//   }
//   toggleAppKeyVisibility(): void { this.showAppKey = !this.showAppKey; }
//   toggleNwkSKeyVisibility(): void { this.showNwkSKey = !this.showNwkSKey; }
//   toggleAppSKeyVisibility(): void { this.showAppSKey = !this.showAppSKey; }
...
//       // ... all other properties remain the same
//       otaaSupported: this.otaaSupported, appKey: this.appKey, devAddr: this.devAddr, nwkSKey: this.nwkSKey, appSKey: this.appSKey,
//       rx1Delay: this.rx1Delay, rx1Duration: this.rx1Duration, rx1DataRateOffset: this.rx1DataRateOffset, rx2Delay: this.rx2Delay, rx2Duration: this.rx2Duration, channelFrequency: this.channelFrequency, dataRate: this.dataRate, ackTimeout: this.ackTimeout,
//       classBSupported: this.classBSupported, classCSupported: this.classCSupported,
//       uplinkDataRate: this.uplinkDataRate, fPort: this.fPort, retransmission: this.retransmission, fCnt: this.fCnt, fCntDownDisable: this.fCntDownDisable, fCntDown: this.fCntDown,
//       adrEnabled: this.adrEnabled, rangeAntenna: this.rangeAntenna,
//       uplinkInterval: this.uplinkInterval, payloadExceedsAction: this.payloadExceedsAction, mType: this.mType, payloadContent: this.payloadContent, base64Encoded: this.base64Encoded,
//       location: {
...
//   cancel(): void { this.router.navigate(['/devices']); }
// }

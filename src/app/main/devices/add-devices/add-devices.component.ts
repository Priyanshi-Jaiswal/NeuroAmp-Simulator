...
export class AddDevicesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    // Initialize map only when the 'Location' tab is active
    // This ensures the map div exists when initMap is called
  ...
  setActiveTab(tabName: string): void {
    ...
    // In a real application, you might show/hide different form sections based on this tab.
    console.log(`Navigated to: ${tabName}`);

    if (tabName === 'Location') {
      // Use a small delay to ensure the map div is rendered and then invalidate size
      setTimeout(() => {
        if (!this.map) {
          this.initMap();
        }
        this.map?.invalidateSize(); // Crucial: Force map to redraw and fill its container
      }, 100);
    }
  }
...
private initMap(): void {
  ...
  this.map = L.map('map').setView([this.latitude, this.longitude], 6); // Set initial view

  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
  }).addTo(this.map);

  // Custom icon for the marker (using default Leaflet icon paths)
  ...
  // Also invalidate size immediately after initialization
   ...
   * This is a placeholder for actual DevEUI generation logic.
   */
  generateDevEUI(): void {
    // A simple placeholder for generating a random 16-character hex string
   ...
   * After saving, it navigates back to the devices list.
   */
  saveDevice(): void {
    if (!this.name || !this.devEUI || !this.selectedGatewayId || !this.region) {
      ...
      this.activeSettingTab = 'General'; // Switch back to General tab
      return;
    }
    const newDeviceData = {
      // General Settings
    ...
    if (!this.deviceCreatedForSession) {
      ...
      this.appService.createDevice(newDeviceData).subscribe({
        next: (response) => {
          ...
          // if (response && response.devEUI) {
          // this.devEUI = response.devEUI; // Assign the backend ID to the local devEUI property
          this.deviceCreatedForSession = true; // Set the flag
          // console.log('Device EUI obtained:', this.devEUI);
          // } 
        },
        error: (error) => {

...
export class EditGatewayComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      if (this.gatewayId) {
        ...
        // Optionally, navigate back to the list page if no ID is found.
  ...
  loadGatewayData(id: string): void {
    this.appService.getSingleGateway(id).subscribe({
      next: (response) => {
        ...
        // Re-initialize map with the loaded data
        setTimeout(() => {
  ...
  selectGatewayType(type: 'virtual' | 'real'): void {
    // This is disabled in the template, so this method is mostly for completeness.
